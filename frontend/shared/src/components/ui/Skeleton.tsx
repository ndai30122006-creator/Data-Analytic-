...
export function Skeleton({ height = 12, width = "100%", style }: { height?: number | string; width?: number | string; style?: React.CSSProperties }) {
  return <div style={{ height, width, background: "rgba(255,42,109,0.1)", borderRadius: 2, animation: "pulse 1.5s infinite", ...style }} />;
}

export function EmptyState({ title, hint, action }: { title: string; hint?: string; action?: React.ReactNode }) {
  return (
    <div style={{ textAlign: "center", padding: 24, border: "1px dashed var(--border)", borderRadius: "var(--radius-card)", background: "rgba(255,42,109,0.03)" }}>
...
export function Toast({ message, type = "info", onClose }: { message: string; type?: "info" | "success" | "error"; onClose?: () => void }) {
  const bg = type === "error" ? "rgba(255,49,49,0.12)" : type === "success" ? "rgba(5,217,232,0.12)" : "rgba(255,42,109,0.12)";
  const border = type === "error" ? "rgba(255,49,49,0.4)" : type === "success" ? "rgba(5,217,232,0.4)" : "rgba(255,42,109,0.35)";
  return (
