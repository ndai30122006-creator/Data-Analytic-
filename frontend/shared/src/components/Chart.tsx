...
function getMockApexOptions(type: string): ApexOptions {
  const common: ApexOptions = {
    ...
    xaxis: { labels: { style: { colors: "#8B98A9", fontSize: "11px" } }, axisBorder: { show: false }, axisTicks: { show: false } } as any,
    yaxis: { labels: { style: { colors: "#8B98A9", fontSize: "11px" } } } as any,
  };
  switch (type) {
    case "kpi":
      return {
        ...
        plotOptions: { radialBar: { dataLabels: { value: { color: "#F1F5F9", fontSize: "16px" } }, track: { background: "rgba(255,255,255,0.07)" } } } as any,
        series: [72],
        labels: ["KPI"],
      } as ApexOptions;
    case "bar":
