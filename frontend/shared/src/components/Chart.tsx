...
function getMockApexOptions(type: string): ApexOptions {
  const common: ApexOptions = {
    ...
    xaxis: { labels: { style: { colors: "#6B8F71", fontSize: "11px" } }, axisBorder: { show: false }, axisTicks: { show: false } } as any,
    yaxis: { labels: { style: { colors: "#6B8F71", fontSize: "11px" } } } as any,
  };
  switch (type) {
    case "kpi":
      return {
        ...
        plotOptions: { radialBar: { dataLabels: { value: { color: "#CFF5D8", fontSize: "16px" } }, track: { background: "rgba(74,222,128,0.1)" } } } as any,
        series: [72],
        labels: ["KPI"],
      } as ApexOptions;
    case "bar":
    ...
    case "line":
      return { ...common, chart: { type: "line" }, stroke: { curve: "smooth", width: 2, colors: ["#4ADE80"] } as any, xaxis: { categories: ["T1", "T2", "T3", "T4"] } as any, series: [{ name: "trend", data: [5, 9, 6, 12] }] } as ApexOptions;
    case "scatter":
