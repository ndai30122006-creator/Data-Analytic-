...
function getMockApexOptions(type: string): ApexOptions {
  const common: ApexOptions = {
    ...
    xaxis: { labels: { style: { colors: "#6b7280", fontSize: "11px" } }, axisBorder: { show: false }, axisTicks: { show: false } } as any,
    yaxis: { labels: { style: { colors: "#6b7280", fontSize: "11px" } } } as any,
  };
  switch (type) {
    case "kpi":
      return {
        ...
        plotOptions: { radialBar: { dataLabels: { value: { color: "#e0e0e0", fontSize: "16px" } }, track: { background: "#1c1c2e" } } } as any,
        series: [72],
        labels: ["KPI"],
      } as ApexOptions;
    case "bar":
    ...
    case "line":
      return { ...common, chart: { type: "line" }, stroke: { curve: "smooth", width: 2, colors: ["#00ff88"] } as any, xaxis: { categories: ["T1", "T2", "T3", "T4"] } as any, series: [{ name: "trend", data: [5, 9, 6, 12] }] } as ApexOptions;
    case "scatter":
