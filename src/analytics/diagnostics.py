        ...
        with st.spinner("Đang phân tích..."):
            ...
            with tabs[0]:
                ...
                if max_vif > 10:
                    ...
                    insight_card(
                        "⚠️", "Moderate Multicollinearity", f"Có VIF={max_vif:.1f} > 5 — cần xem xét", "warning"
                    )
                else:
                    insight_card(
                        "✅", "No Multicollinearity", "Tất cả VIF < 5 — không có đa cộng tuyến đáng kể", "good"
                    )
                ...
                try:
                    ...
                    if lm_p < 0.05:
                        insight_card(
                            ...
                            f"p = {lm_p:.6f} < 0.05 — Phương sai sai số thay đổi",
                            "warning",
                        )
                    else:
                        insight_card(
                            "✅", "Homoskedasticity", f"p = {lm_p:.4f} ≥ 0.05 — Phương sai sai số đồng đều", "good"
                        )
                    fig = go.Figure()
                    fig.add_trace(
                        go.Scatter(
            ...
            with tabs[2]:
                ...
                dw_msg = (
                    f"⚠️ Có tự tương quan dương (DW gần 0)"
                    if dw < 1.5
                    else (
                        f"✅ Không có tự tương quan (DW gần 2)"
                        if 1.5 <= dw <= 2.5
                        else f"⚠️ Có tự tương quan âm (DW gần 4)"
                    )
                )
                insight_card(
                    "📊", "Interpretation", f"DW = {dw:.4f}. {dw_msg}", "good" if 1.5 <= dw <= 2.5 else "warning"
                )
                fig = go.Figure()
                fig.add_trace(
                    go.Histogram(x=residuals, nbinsx=30, marker_color="#818cf8", opacity=0.7, name="Residuals")
                )
                fig.update_layout(
                    title="Distribution of Residuals", height=300, xaxis_title="Residual", yaxis_title="Frequency"
                )
                apply_theme(fig)
                st.plotly_chart(fig, use_container_width=True)
            ...
            with tabs[3]:
                ...
                coef_df = pd.DataFrame(
                    {"Feature": ["Intercept"] + features, "Coefficient": [model.intercept_] + list(model.coef_)}
                )
                st.dataframe(coef_df, use_container_width=True)
                fig = make_subplots(rows=1, cols=2, subplot_titles=("Q-Q Plot (Normality)", "Residuals vs Fitted"))
                (osm, osr), (slope, intercept, _r) = probplot(residuals, dist="norm")
                fig.add_trace(
                    go.Scatter(x=osm, y=osr, mode="markers", marker=dict(color="#818cf8", size=4), showlegend=False),
                    row=1,
                    col=1,
                )
                fig.add_trace(
