"""Advanced Statistics — via core/statistical_tests (P1 dedup)."""
...
try:
    from scipy.stats import kstest, normaltest, probplot, shapiro
...
def render_advanced_stats_tab(df, num, cat):
    if not SCIPY_AVAIL:
        st.warning("Cài đặt: pip install scipy")
        return
    if not validate_df(df, num, cat):
        return

    st.markdown("### Thống kê nâng cao (Book Ch.2-3) — via core")
    st.caption("Kiểm định giả thuyết, Effect Size — single source `core`")
...
def _render_ttest_2sample(df, num, cat):
    ...
    if st.button("Run", key="ht_run_adv"):
        ...
        c4.metric("Effect", "Lớn" if abs(res["cohens_d"]) > 0.8 else "TB" if abs(res["cohens_d"]) > 0.5 else "Nhỏ")
        insight_card(
            "📊",
            "Kết quả",
            f"p = {res['p_value']:.6f} {'< 0.05 → Có khác biệt' if res['significant'] else '≥ 0.05 → Không khác biệt'}. Cohen's d = {abs(res['cohens_d']):.3f}",
            "good" if res["significant"] else "info",
        )
        fig = go.Figure()
        ...
        fig.update_layout(
            title=f"So sánh {val_col}: {g1} vs {g2} (p={res['p_value']:.4f}, d={abs(res['cohens_d']):.3f})"
        )
        apply_theme(fig)
...
def _render_ttest_1sample(df, num):
    ...
    if st.button("Run", key="ht_1s_run_adv"):
        ...
        insight_card(
            "📊",
            "Kết quả",
            f"Mean = {np.mean(s):.4f} vs μ₀ = {mu0:.4f}. Cohen's d = {abs(res['cohens_d']):.3f}. {'Có khác biệt' if res['significant'] else 'Không khác biệt'}.",
            "good" if res["significant"] else "info",
        )
...
def _render_ttest_paired(df, num):
    ...
    if st.button("Run", key="ht_p_run_adv"):
        s = df[[c1, c2]].dropna()
        res = run_ttest_paired(s[c1].values, s[c2].values)
        c1, c2 = st.columns(2)
        c1.metric("t-statistic", f"{res['statistic']:.4f}")
        c2.metric("p-value", f"{res['p_value']:.6f}")
        insight_card(
            "📊",
            "Kết quả",
            f"Paired t-test: p = {res['p_value']:.6f}, Cohen's d = {abs(res['cohens_d']):.3f}",
            "good" if res["significant"] else "info",
        )
...
def _render_anova(df, num, cat):
    ...
    if st.button("Run", key="ht_anova_run_adv"):
        ...
        c2.metric("p-value", f"{res['p_value']:.6f}")
        c3.metric("η²", f"{res['eta_squared']:.4f}")
        insight_card(
            "📊",
            "Kết quả ANOVA",
            f"p = {res['p_value']:.6f}. η² = {res['eta_squared']:.4f}",
            "good" if res["significant"] else "info",
        )
...
def _render_mannwhitney(df, num, cat):
    ...
    if st.button("Run", key="ht_mw_run_adv"):
        ...
        c1, c2, c3 = st.columns(3)
        c1.metric("U-statistic", f"{res['statistic']:.2f}")
        c2.metric("p-value", f"{res['p_value']:.6f}")
        c3.metric("r", f"{res['r']:.4f}")
        insight_card(
            "📊",
            "Kết quả",
            f"Mann-Whitney: p = {res['p_value']:.6f}, r = {res['r']:.4f}",
            "good" if res["significant"] else "info",
        )
...
def _render_kruskal(df, num, cat):
    ...
    if st.button("Run", key="ht_kw_run_adv"):
        ...
        c2.metric("p-value", f"{res['p_value']:.6f}")
        insight_card(
            "📊", "Kết quả", f"Kruskal-Wallis: p = {res['p_value']:.6f}", "good" if res["significant"] else "info"
        )
...
def _render_chisquare(df, cat):
    ...
    if st.button("Run", key="ht_cs_run_adv"):
        ...
        c4.metric("Cramer's V", f"{res['cramers_v']:.4f}")
        insight_card(
            "📊",
            "Kết quả",
            f"Chi-Square: p = {res['p_value']:.6f}. Cramer's V = {res['cramers_v']:.4f}",
            "good" if res["significant"] else "info",
        )
        fig = px.imshow(ct, text_auto=True, title="Contingency Table", color_continuous_scale="Blues", aspect="auto")
        apply_theme(fig)
        st.plotly_chart(fig, use_container_width=True)
...
def _render_normality(df, num):
    ...
    if st.button("Check Distribution", key="norm_run"):
        ...
        fig.add_trace(go.Histogram(x=s, nbinsx=40, marker_color="#8B5CF6", opacity=0.7), row=1, col=1)
        (osm, osr), (slope, intercept, _r) = probplot(s, dist="norm")
        fig.add_trace(go.Scatter(x=osm, y=osr, mode="markers", marker=dict(color="#8B5CF6", size=4)), row=1, col=2)
        fig.add_trace(
            go.Scatter(x=osm, y=slope * osm + intercept, mode="lines", line=dict(color="#EC4899", dash="dash")),
            row=1,
            col=2,
        )
        fig.add_trace(go.Box(y=s, marker_color="#06B6D4", boxmean=True), row=1, col=3)
