...
try:
    from src.ui.theme import metric_card, status_badge, gradient_text, icon
except ImportError:
...
def render_bento_dashboard(df, num, cat):
    ...
    with col1:
        st.markdown(metric_card("Tổng quan dữ liệu", f"{len(df):,} dòng · {df.shape[1]} cột",
                                 icon="database", color="primary"), unsafe_allow_html=True)
    with col2:
        if num:
            st.markdown(metric_card("Điểm TB", f"{df[num[0]].mean():.2f}",
                                     f"Min: {df[num[0]].min():.1f}", "trending", color="accent"), unsafe_allow_html=True)
        else:
    ...
    with col3:
        if num:
            ...
            st.markdown(metric_card("Tỷ lệ đạt", f"{pass_rate:.1f}%",
                                     f"≥ 5.0", "check", color="success"), unsafe_allow_html=True)
        else:
    ...
    with col_right:
        ...
        if num:
            fig = px.histogram(df, x=num[0], nbins=30, marginal="box",
                             title=f"{num[0]}", color_discrete_sequence=["#1E40AF"])
