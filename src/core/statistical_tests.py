...
def run_ttest_onesample(s: np.ndarray, mu0: float) -> Dict[str, Any]:
    ...
    return {"statistic": stat, "p_value": p, "significant": p < SIGNIFICANCE_LEVEL, "test_name": "One-Sample T-test"}


def run_ttest_paired(before: np.ndarray, after: np.ndarray) -> Dict[str, Any]:
    """Paired t-test."""
    stat, p = scipy_stats.ttest_rel(before, after)
    return {"statistic": stat, "p_value": p, "significant": p < SIGNIFICANCE_LEVEL, "test_name": "Paired T-test"}
...
def run_kruskal(*groups: np.ndarray) -> Dict[str, Any]:
    ...
    return {"statistic": stat, "p_value": p, "significant": p < SIGNIFICANCE_LEVEL, "test_name": "Kruskal-Wallis"}
...
def run_bootstrap(data: np.ndarray, n_iter: int = 1000, conf_level: int = 95, stat_func=np.mean) -> Dict[str, Any]:
...
def run_two_proportion_ztest(successes_a: int, total_a: int, successes_b: int, total_b: int) -> Dict[str, Any]:
...
def calculate_sample_size(baseline_pct: float, effect_pct: float, alpha: float = 0.05, power: float = 0.8) -> int:
    ...
    z_b = norm.ppf(power)
    n = (
        (z_a * np.sqrt(2 * (p1 + p2) / 2 * (1 - (p1 + p2) / 2)) + z_b * np.sqrt(p1 * (1 - p1) + p2 * (1 - p2))) ** 2
    ) / ((p2 - p1) ** 2)
